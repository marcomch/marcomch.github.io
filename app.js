...
function procesarResultado(ganancia, contractType) {
    trading.isTrading = false;
    stopTradeMonitor();

    // BUG 1 FIX: trading.balance NO se modifica aquí.
    // El saldo real es actualizado exclusivamente por el evento data.balance
    // del WebSocket de Deriv, evitando doble contabilización.
    ...
    if (el.lastTrade) {
        // BUG 2 FIX: Se usa el tipo real del contrato (CALL/PUT) en lugar de
        // inferirlo por el resultado, lo que causaba etiquetas invertidas.
        const tipoLabel = contractType === 'CALL' ? 'CALL' : contractType === 'PUT' ? 'PUT' : (ganancia > 0 ? 'CALL' : 'PUT');
        el.lastTrade.innerHTML = ganancia > 0
            ? `<span style="color:#27ae60">${tipoLabel} +$${ganancia.toFixed(2)}</span>`
            : `<span style="color:#e74c3c">${tipoLabel} -$${Math.abs(ganancia).toFixed(2)}</span>`;
    }
...
function conectarDerivAPI() {
    ...
    trading.derivWs.onmessage = msg => {
        ...
        if (data.buy && data.buy.contract_id) {
            ...
            // BUG 2 FIX: Se guarda contract_type para mostrar correctamente CALL/PUT en lastTrade
            const contractType = data.buy.contract_type || '';
            trading.currentContract = { contractId, stake, contractType, timestamp: Date.now(), status: 'open' };
            // BUG 1 FIX: No se descuenta el stake manualmente.
            // El saldo real es actualizado por data.balance (suscripción activa),
            // que es la fuente de verdad de Deriv. El descuento manual causaba
            // doble penalización en el balance local.
            updateTradingDisplay();
        ...
        if (data.proposal_open_contract) {
            ...
            if (contract.is_sold) {
                ...
                // BUG 2 FIX: Se captura el tipo de contrato ANTES de anular currentContract
                const savedContractType = trading.currentContract?.contractType
                    || contract.contract_type
                    || '';
                setTimeout(() => {
                    procesarResultado(profit, savedContractType);
        ...
        if (data.error) {
            if (data.error.code === 'ContractBuyValidation' || data.error.code === 'InvalidContract') {
                // BUG 1 FIX: Ya no se suma el stake de vuelta manualmente porque
                // nunca se descontó. El balance real llega via data.balance.
                trading.isTrading = false;
